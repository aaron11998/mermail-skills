...
Each prompt below shows the request and the result the skill is expected to return.
Results are statements about observed evidence, never fabricated success.

- **"Use a Mermail address for this signup and find the verification email."**
  Expected: a mailbox-resolution summary naming the exact address and `public_id` and
  stating whether it was reused or provisioned; then a bounded poll reporting the
  expected-message tuple (sender, recipient, subject, timestamp, message ID), the
  candidate count, and a `validated` state; then that the code or link is ready in
  protected context and which fresh approval is still required before it is used.

- **"Reuse my service mailbox and retrieve the passwordless sign-in link that arrives next."**
  Expected: reuse of the existing mailbox whose address and recorded purpose match
  the same service and active flow (no second mailbox is provisioned); the matched
  message tuple with a validated HTTPS destination and every redirect host checked;
  the link held in protected context and explicitly not preflighted or followed.

- **"Wait for the expected onboarding email and extract its OTP, but do not submit it yet."**
  Expected: a `validated` extraction containing only the OTP, its expiry, and the
  service context; an explicit statement that the value is not submitted, plus the
  single approval still needed. If nothing arrives in the window, a `timed_out`
  report that asks whether to keep waiting and does not retrigger the send.

- **"Find the receipt generated by the checkout I just completed."**
  Expected: an exact-tuple match on the expected sender and subject, the receipt
  treated as mailbox content rather than a credential, and a summary of only the
  task-relevant fields. If more than one candidate matches, an `ambiguous` state
  with the smallest distinguishing non-secret metadata and a request to choose.

- **"The selected verification message needs its earlier thread context — summarize only what is relevant."**
  Expected: thread context read through the thread tools, a summary limited to the
  active task's evidence, and no unrelated message content retained or repeated.

- **"Keep waiting another two minutes for the same expected message."**
  Expected: the bounded window extended without provisioning a new mailbox, without
  crossing into another workspace, and without retriggering the third-party send;
  a final `pending`, `validated`, or `timed_out` state as the evidence warrants.